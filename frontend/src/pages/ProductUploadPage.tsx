...
const ProductManagePage = () => {
    ...
    const clickNextPage = () => setPage(p => Math.min(dummyTotalPages, p + 1));
    const clickPrevPage = () => setPage(p => Math.max(1, p - 1));
    const onClick = (e: any) => setPage(Number(e?.currentTarget?.value) || 1);
    ...
    return (
        <>
            ...
            <div style={{marginTop: 30, textAlign: "center"}}>
                <h2>임시 페이징: {page} / {dummyTotalPages}</h2>
            </div>
        </>
    );
};
