...
export function useSelectionRect(opts: UseSelectionRectOptions = {}) {
  ...
  const hit = useCallback((sx: number, sy: number):
    ...
    if (inX && inY) {
      ...
      if (bottom && left) return { where: "handle", handle: "sw" };
      if (top) return { where: "handle", handle: "n" };
      if (right) return { where: "handle", handle: "e" };
      if (bottom) return { where: "handle", handle: "s" };
      if (left) return { where: "handle", handle: "w" };
      return { where: "inside" };
    }
  ...
  const moveDrag = useCallback((sx: number, sy: number) => {
    ...
    const squareIfShift = (x: number, y: number, w: number, h: number) => {
      if (!shiftRef.current) return { x, y, w, h };
    ...
    if (m.type === "move") {
      ...
      setRect({ x: m.base.x + dx, y: m.base.y + dy, w: m.base.w, h: m.base.h });
      return;
    }
    if (m.type === "resize") {
      const dx = sx - m.ox, dy = sy - m.oy;
