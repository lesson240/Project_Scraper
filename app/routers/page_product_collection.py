...
from app.services.service_collect import ExecuteSpecialToday
...
async def special_today(
    background_tasks: BackgroundTasks, input_data: SpecialTodayModel
):
    logger.info(f"Received data: {input_data.json()}")

    redundant = input_data.redundant
    ...
    background_tasks.add_task(
        run_special_today_task, site_key, redundant, group_name, memo
    )

    # JSON 응답 생성
    return JSONResponse(content={"message": "Task started"}, status_code=202)


async def run_special_today_task(site_key, redundant, group_name, memo):
    special_today = ExecuteSpecialToday(site_key, redundant, group_name, memo)
    try:
        result = await special_today.run()
        logger.info(f"SpecialToday task completed successfully: {result}")
    except Exception as e:
        logger.error(f"SpecialToday task failed: {e}")
